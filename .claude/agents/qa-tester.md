...
- **Cross-page state coherence is mandatory, not optional.** Per-page tests all passed while the workspace selector lied to the user for weeks (header chip vs dashboard selector were two unsynced states; nothing persisted across reload; the chip misreported the workspace on project pages). For ANY stateful/global UI element (active workspace, board/filter selection, current sprint, anything shown in the header), you must test: (a) change it on surface A → is surface B consistent? (b) navigate away and back → still correct? (c) full reload → does the selection survive? (d) delete/rename the underlying entity → does every surface heal? (e) deep-link directly into a scoped page → do the global indicators reflect where you actually landed? See `e2e/workspace-switcher.spec.ts` for the canonical shape of this suite.
