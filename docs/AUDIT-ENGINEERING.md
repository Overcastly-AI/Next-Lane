...
- **RESOLVED (2026-07-06, Hardening Night wave 2, task #93):** new shared `apps/api/src/common/ssrf-safe-fetch.ts` — `ssrfSafeFetch()` resolves DNS exactly once and pins the connection to that address via a custom undici `Agent` `connect.lookup` (new `undici` direct dependency); all three call sites (`webhooks.service.ts` delivery, `GithubClient`, `GitlabClient`) migrated onto it. 15 new tests in `ssrf-safe-fetch.spec.ts`, including a real-socket rebind simulation (mocked `dns.promises.lookup` answers public-then-private; the request still hits the first-vetted address and the lookup is proven called exactly once).
...
- **RESOLVED (2026-07-06, Hardening Night wave 2, task #93):** root `pnpm.overrides` now forces `multer: "^2.2.0"` and (the one other HIGH, also cheap/same-major) `lodash: "^4.18.0"`. `pnpm audit --prod`: 5 high → **0 high** (10 → 7 moderate). `pnpm why multer`/`pnpm why lodash` confirm every resolution path lands on the patched version. Live-verified with a real multipart `curl` round-trip against a running API (workspace logo + issue attachment, byte-identical upload/download through the patched `FileInterceptor`/multer path). Remaining 7 moderates (`js-yaml` ×2, `qs`, `@nestjs/core`, `file-type` ×2) intentionally deferred — `@nestjs/core`'s fix needs a Nest v10→v11 major bump and `file-type`'s needs v20+ (ESM-only, already its own ticket per this risk's own Fix note); not blindly bumped. `pnpm audit --prod --audit-level=high` as a CI gate (Ideation #3) is still open.
