...
- ✅ Public read-only share link — `ShareToken` model (SHA-256 hashed `nls_` token); ADMIN mint/list/revoke (`POST|GET|DELETE /projects/:id/share-tokens`); unauthenticated `GET /api/public/board/:token` (rate-limited, `@Public()` bypass, tenant-isolated); `/share/:token` read-only board view (no DnD, no create, read-only banner); `ShareSection` in Project Settings; migration `20260627235000_add_share_token`; 17 unit tests + 14 e2e (desktop + mobile) — all green (2026-06-27)
...
Public read-only share link shipped 2026-06-27: `ShareToken` model (migration `20260627235000_add_share_token`; SHA-256 hashed `nls_` prefix token); ADMIN mint/list/revoke at `POST|GET|DELETE /projects/:id/share-tokens`; unauthenticated `GET /api/public/board/:token` (rate-limited 60/min, bypasses JwtAuthGuard via `@Public()`, tenant-isolated); `/share/:token` standalone React page (no DnD, no create, no drawer, read-only banner); `ShareSection` in Project Settings; 17 unit tests (all green); 14 e2e tests (desktop + mobile, all green); 320 total API tests.

Next build order: markdown rendering in descriptions/comments (M, P2) → inline card status transition (S, P2) → PAT scopes (M, P2) → remaining perf (batch inserts, slim planning endpoint, board-overview prefetch) + P3 ideas (sprint retros, issue templates).
