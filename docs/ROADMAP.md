...
- ✅ 12-factor config: everything is env-driven (`DATABASE_URL`, `JWT_SECRET`, `CORS_ORIGINS`, `THROTTLE_*`, `WEBHOOK_ALLOW_PRIVATE`, `LOG_LEVEL`). The build-time `VITE_API_URL` gap is closed for K8s by the Helm chart's same-origin nginx reverse-proxy (`web.apiMode: same-origin`) — one web image works across environments without rebuilds.
...
- ✅ **Web runtime config**: `web.apiMode: same-origin` (default) ships an nginx ConfigMap that reverse-proxies `/api` + `/socket.io` to the API Service, so one web image (built with empty `VITE_API_URL`) works across environments without rebuilds; `external` mode documented for split-origin deploys.
