...
- ✅ **Doc templates for Pages (2026-07-30, founder directive: "for my docs I would like templates that I can preload with certain markdown formats")** — reusable markdown skeletons for new pages, at BOTH scopes (founder chose "Both"). New `PageTemplate` model mirrors `Page`'s scoping exactly (`workspaceId` always set, `projectId` nullable): a workspace-wide template is offered when creating a page anywhere in the workspace *including inside a project*, and a project can add its own on top — the project's rows sort first in the picker, so a same-named project template reads as a deliberate override. **Six built-in starters ship seeded** (Meeting notes, Decision record/ADR, Runbook, Spec/PRD, Retrospective, How-to guide) as ordinary editable/deletable rows, not fixtures. **Placeholders** (`{{date}} {{time}} {{datetime}} {{year}} {{month}} {{day}} {{title}} {{author}}`) are substituted server-side at creation by a shared renderer (`packages/shared/src/page-template.ts`) that the web editor also uses for its live preview, so the two can't drift; an *unknown* token is left verbatim rather than blanked (a fill-me-in marker must survive) and the editor warns about it before the template ships. **Three correctness details worth recording.** (1) Name uniqueness uses two PARTIAL unique indexes, not `@@unique([workspaceId, projectId, name])` — Postgres treats NULLs as distinct, so the composite form would have placed *no* constraint on workspace-wide rows, the exact collision worth preventing; proved with SQL that two workspace "Runbook"s are rejected while a project override is allowed. (2) Starter seeding is keyed on a new `Workspace.pageTemplatesSeededAt` MARKER, never on "this workspace has no templates" — keyed on the count, a workspace whose owner deleted all six would have them resurrected on the next API restart; verified live (a null-marker workspace got 6 on boot, a purged one stayed at 0). (3) The marker is claimed with a conditional `updateMany`, so two replicas booting together cannot double-seed. Rendered titles are re-validated against the `[ ] |` wiki-link ban AFTER substitution, because `{{author}}` expands to a user-controlled display name. **Surface:** 8 REST routes gated by `pages:*` (not `projects:*` — a template is page content, so a wiki-scoped agent shouldn't need project admin), a template picker in the New-page modal (desktop + mobile), a management section in both project and workspace settings, and 2 MCP tools (`list_page_templates`, `create_page_from_template`) taking the server to **126 tools (61 read / 65 write)**. **Gates:** API unit 2126/2126, shared 210/210, MCP 155/155, web 11/11, integration **464/464** including 9 new cross-tenant doc-template rows and the 8 new routes in the PAT-scope matrix; new `page-templates.spec.ts` 4×2 = 8/8 plus 42/42 existing pages e2e and 68/68 settings/workspace e2e, all desktop+mobile; `tsc --noEmit` clean across api/web/mcp. Two real bugs were caught by running the real artifact rather than the tests: the mutations double-encoded their JSON bodies (typecheck and units both passed it), and the picker's `grid` overflowed the modal and clipped every description mid-word on mobile — both fixed, the second with a geometry assertion so it can't regress. [founder directive 2026-07-30]
- ✅ **Activity log now updates from your own change, not a socket echo (2026-07-31)** — chased down the intermittent `issue-detail.spec.ts:53` mobile failure and found the test was right and the product wrong, in two layers. **(1)** No mutation ever invalidated `qk.activity`; the only refresh path was the `issue.updated` realtime handler, so observing your own write depended on a round trip through the socket coming back — silently broken without a Redis adapter, behind a proxy that drops WebSockets, or during a reconnect. User-visible symptom: change a status, Activity doesn't show it. Invalidation added to `useUpdateIssue`, `useBulkUpdateIssues`, `useMoveIssue` and `useToggleIssueLabel`. **(2)** That alone only took the failure rate ~17% → ~4%, because an invalidation landing while an activity fetch is already in flight is DEDUPED onto that request — which was issued before the write, resolves with pre-change rows, and marks the query fresh so nothing refetches. Instrumented trace showed exactly one request and one response across the whole sequence. Fixed with `cancelRefetch: true`, centralised in a documented `invalidateIssueActivity()` helper so the flag can't be silently dropped. **Proven both directions** at 40 trials / 8 workers on identical builds: `true` → 0/40, `false` → 2/40, no invalidation → ~4/24 baseline. An earlier filing of this bug blamed `useOverlay` closing the drawer; that was wrong (no outside-click close exists, and the snapshots show the drawer open) and is corrected in `docs/BACKLOG.md`. [found while driving #59 CI green]
- ✅ **Every file upload was 500ing in Docker and Kubernetes (2026-07-31, founder report)** — workspace-logo and issue-attachment uploads both moved the multer temp file with a bare `fs.renameSync`, which cannot cross a filesystem boundary. Every real deployment crosses one (multer → `/tmp`; `UPLOADS_DIR` → a named volume in compose, a separate `emptyDir`/PVC in Helm), so uploads failed with an unhandled `EXDEV` → blunt 500, on **every** containerised install, while passing on any dev host where the two paths share a disk. Reproduced verbatim by pointing `UPLOADS_DIR` at a tmpfs, fixed with a `moveUploadedFile()` helper (atomic rename, `copyFileSync`+`unlink` fallback on EXDEV only), and re-verified end-to-end: logo 500 → 201 with a byte-identical round trip, attachments likewise. The regression test runs on a real second device and asserts the premise so it can't degrade into a meaningless same-device move. Third defect this week from the same root class — **the artifact that ships was never the artifact that was tested** — which is the case for the first-run smoke gate still open in the v1 criteria. [founder report 2026-07-31]
- ✅ **Object storage — S3-compatible, so Ceph works for both Docker and Kubernetes (2026-07-31, founder directive)** — uploads go through a `StorageDriver` interface: `local` (default, unchanged behaviour and no migration) or `s3`. Deliberately **S3-compatible rather than Ceph-native**: Ceph RGW speaks S3, so one driver also covers MinIO, AWS S3, R2 and Wasabi, whereas a Ceph-native integration would exclude all of them and force a single-host compose user to run a Ceph cluster. Compose ships MinIO behind a profile (default `up` unchanged); Kubernetes documents Ceph via Rook, including reading credentials straight from a `CephObjectStoreUser` secret with remapped key names. **This also lifts the multi-replica limit** — `storage.driver=s3` means every api pod shares one bucket, so the ReadWriteMany guard now stands down automatically and recommends s3 as the primary fix. The integration test (in-process `s3rver`, real S3 wire protocol, no Docker) caught a defect that would have shipped: the AWS SDK's default `aws-chunked` streaming checksums are unparsed by MinIO and several Ceph RGW versions, which store the chunk framing *as the file's content* — silently corrupting every upload. Pinned to `WHEN_REQUIRED`. Evidence: API unit 2147/2147, chart verified across 9 configurations, and a live round trip against a real S3 server (byte-identical, nothing written to local disk) plus the same trip re-verified on the local driver. [founder directive 2026-07-31]
- ✅ **Images in documentation pages (2026-07-31, founder directive: "add images to our documentation")** — paste or drag an image straight into a page body and it uploads to the page and renders inline. Rides the storage driver above, so the bytes land on local disk or in Ceph/S3 with no extra wiring. **The design decision worth recording is how an image is addressed.** The body stores `![alt](nl-image:<id>)` — an app-internal scheme, not a URL. A stored absolute URL bakes the deployment origin into user content (move an install from `localhost` to a domain, or export a page, and every image breaks), and worse, it would have to be fetchable without an `Authorization` header — which would make an image embedded in a private page *less private than the page*. Instead the renderer resolves the scheme at display time, fetching each image with the reader's own token and swapping in a `blob:` URL, so an image inherits the page's permissions exactly; there is no second ACL to drift. The founder chose this (authenticated blob) over public-unguessable URLs and signed URLs. **Details:** new `PageImage` model (cascade on page delete, `SetNull` on uploader delete); 4 REST routes gated by `pages:*` for the same reason the doc-template routes are; PNG/JPEG/GIF/WebP only with magic-byte verification (no SVG — it is an active document and these render inline); 10 MB cap shared between API and editor from one constant in `packages/shared`; delete removes the blob *before* the row, since the row is the only pointer to it. The sanitizer change is the risky part and is fenced accordingly: DOMPurify's URI allowlist gains `nl-image` and nothing else, the `src` must match a bare id, and `nl-image:` is stripped from anchors so it can never be a link target. **Not exposed via MCP, deliberately** — the operation is "paste these bytes", which an agent has no good way to originate, and the read side would hand an agent a binary blob it cannot use; agents already get the page body, where the reference is legible as text. **Gates:** API unit 2164/2164 (17 new, covering both directions of the privacy claim), web unit 23/23 (12 new; verified failing when the sanitizer change is reverted), PAT-scope coverage 6/6 with the 4 new routes, and a new `page-images.spec.ts` (2×2 desktop+mobile) that pastes a real PNG into the real stack and asserts it decodes (`naturalWidth > 0`, which a corrupted upload would fail), survives a reload, and is refused byte-for-byte to another tenant and to an anonymous caller while the owner gets it back identical. 52/52 existing Pages e2e still green. [founder directive 2026-07-31]
- ✅ **Agents can author doc templates, not just use them (2026-07-31, founder: "MCP should be able to make doc templates")** — the doc-templates slice shipped with `list_page_templates` + `create_page_from_template`, which let an agent *consume* the house format but not *establish* it: a founder who asked their agent to "set up a runbook template for this workspace" hit a wall. Four new MCP tools close it — `create_page_template`, `get_page_template`, `update_page_template`, `delete_page_template` — taking the server to **130 tools (62 read / 68 write)**. `get_page_template` exists separately from the list because the list is compacted and omits the markdown body, and the update is a partial patch: an agent that edits `content` without reading it first replaces the template wholesale. That hazard, and the fact that `create_page_from_template`'s `title` REPLACES `titleTemplate` rather than feeding `{{title}}` into it, are both stated in the tool descriptions — and both were things I had *wrong* until a live round trip against the running API corrected me, which is why that round trip now ships as a `NL_LIVE_API`-gated test rather than living only in a commit message. Scope selection mirrors `list_page_templates` (exactly one of `workspaceId`/`projectId`, rejected with a sentence rather than a schema dump) and an ambiguous scope never reaches the API. Writes require workspace ADMIN and `pages:write`, unchanged from the REST layer. **Gates:** MCP 162/162 (16 new) + the live test green against a real instance; `tsc --noEmit` clean; every "126 tools" claim across README, `apps/mcp/README.md` and the docs site reconciled to the runtime count (the old 61/65 split was accurate — verified by counting `allTools` at runtime rather than grepping, which was off by one). [founder directive 2026-07-31]
- ✅ **The docs nav is draggable — resize the panel, drag pages to reorder or nest (2026-08-01, founder: "on the docs pages the inner left doc nav should be draggable")** — the founder chose BOTH readings of "draggable", so both shipped. **(1) Resize:** the divider between the page tree and the document is now a drag handle; the tree was a fixed 256px, which truncated any real page title. New `ui/ResizeHandle` primitive (pointer-capture so a fast drag can't slip off the 6px hit area and die mid-gesture; `role="separator"` with Arrow/Home/End keys, because a resize only mouse users get is not a feature everyone gets) plus `lib/usePersistentWidth`, which clamps a stored width rather than trusting it — an out-of-range value from an older build could otherwise render the panel at 4px with no way back. Built as a primitive, not inline, because the board and issue drawer have the same fixed-panel problem. **(2) Drag to reorder/nest:** drop on a row's top or bottom quarter to reorder, its middle half to nest as a child. Native HTML5 DnD rather than dnd-kit — the board reorders a flat list, but a tree needs three outcomes per row, and the row's own geometry gives that directly. The up/down buttons **stay**: drag has no keyboard equivalent, so removing them would take reordering away from keyboard users entirely. **Two real bugs this surfaced, neither in the new code.** First, gating `onDragOver` on React state set in `dragstart` meant `preventDefault()` never ran within a single tick, so the browser refused the drop — now ref-driven, which also fixes a fast real-world drag, not just the test. Second and worse: `useMovePage.onSettled` deliberately does NOT refetch the tree on success (a refetch between rapid clicks used to swallow moves), which makes `optimisticallyReorderTree` the *only* thing that updates the UI — and it explicitly did not handle re-parenting. A nest therefore persisted server-side and never appeared. Taught it to re-parent, with 7 new unit tests including the refuse-to-move-into-own-subtree case. **Gates:** new `docs-nav-drag.spec.ts` 5/5 against the real stack (resize persists across reload, keyboard resize clamps at both bounds, nest survives reload, edge-drop reorders without nesting, a page cannot be dropped into its own subtree); all 52 existing Pages e2e green including the up/down reorder spec; web unit 33/33; `tsc --noEmit` clean. [founder directive 2026-08-01]
- ✅ **Screenshots reshot, and reshooting made reproducible (2026-08-01, founder: "the docs need new screenshots and updated")** — the shipped set was captured 2026-07-03 and had drifted; two entries (`pages-graph-desktop`/`-mobile`) had sat marked "planned — not yet captured" since the knowledge graph shipped. **The root cause was process, not neglect:** the dressed "Nova Analytics" dataset existed only in whoever staged it last, so a reshoot meant rebuilding a workspace from memory. Now it is `apps/api/prisma/seed-screenshots.ts` (a showroom dataset — WIP limits sitting exactly at capacity, a blocked card, colour rules that actually match, a half-burned Sprint 14) plus `apps/web/e2e/screenshots.capture.ts` and its own Playwright config at 1440x900 @2x / 393x852 @2x. Named `.capture.ts`, so the test config provably cannot pick it up. **Three bugs the first capture exposed, each a case of the seed lying about the product:** a board `updateMany` matched zero rows because the API creates the default board lazily, so the shot showed a Kanban "Main Board" with no colour rules; the graph rendered six isolated dots because `PageLink` edges are synced by the service on save and a direct Prisma write never triggers that (fixed by resolving links with the *same* shared parser the app uses); and the tree shot truncated every page title at the 240px default, which is the exact papercut the new resize handle exists to fix. **26 shots reshot — every surface in BOTH light and dark** (founder: "everything in dark mode"), including 5 new Docs ones; the wiki behind the graph shots is now **101 pages / 358 wiki-link edges** across seven clustered sections rather than six (founder: "the graph looks rather empty") — a six-node graph photographs as an empty canvas and says nothing about what a force-directed view is for, since the clusters, hubs and bridges only appear at scale; 8 carried over unchanged and **explicitly listed as such** in `docs/screenshots/README.md` rather than silently implied current — the capture script doesn't yet stage a four-gadget dashboard or a Gantt. Also corrected two stale doc claims: features.md still said issue<->page cross-linking was "on the roadmap, not shipped yet" (it shipped), and doc templates were undocumented entirely. [founder directive 2026-08-01]
- ✅ **Design elevation pass — board depth, a capacity rail, toolbar hierarchy, nav IA (2026-08-01, founder: "I want something before v1.0.0. It should look a whole lot better")** — run through the `frontend-design` skill per the standing directive. **The board's three real defects, none of them taste:** (1) `--nl-canvas` and `--nl-ink-50` were the SAME hex, so the page and the lane columns on it were literally the same colour and the board read as one grey field with hairlines — canvas now recedes to `#eef1f6`, giving the DISPATCH "ink on paper" thesis its three tiers (desk < lane < card); (2) the column accent was a `border-t-2` that drew identically whether a lane held one card or was over its WIP limit — it is now a **capacity rail** that fills with `count/limit` and turns red when over, so the board's load reads before any word does, and lanes with NO limit get a flat track rather than a full bar that would assert a limit they don't have; (3) sixteen toolbar controls of identical weight wrapped into three ragged rows, leaving `+ Create issue` at the bottom-right under two rows of filter chrome — now split by INTENT into Act and Refine, with export/import/colours demoted to icon ghosts so exactly one element is solid. **The mobile regression this caused was caught by an existing guard, not by me:** promoting the action cluster added 40px between search and filters and pushed the Type filter from y=173 to y=217; there is no "top right" on a phone, so the Act row is `display: contents` below `sm` and the cluster orders itself last — one DOM, no duplication. **Nav IA:** four of the five sidebar-audit findings closed — a Home row (there was previously NO route back to the landing page from the nav), a heading on the one group of four that lacked one, `Workspace docs` to end two rows both labelled `Docs` pointing at different content trees, and the `Insights` nav item reconciled with the `My Analytics` heading it opened. Two findings left open and recorded rather than dropped: the sidebar/tab-bar view mismatch (a refactor, not a rename) and the skip-link/tab-order work. **Gates: the full e2e suite 1140/1140 passed, 0 failed** (desktop + mobile), plus targeted runs of 96/96 board and 68/68 nav specs during development; all 26 screenshots re-shot in both themes. [founder directive 2026-08-01]
...
- ✅ **R1 — recall that returns answers, not titles** (2026-07-30, `docs/RESEARCH-AGENT-MEMORY.md` §4.1/§5-R1) — search was computing `ts_rank` over indexed bodies and then returning **titles only**, so judging a hit required a follow-up full-record fetch (up to 256 KiB per page): measured **19,810 B across 4 calls** to answer "what do we know about rate limiting?". Three changes, one behaviour: (1) **`ts_headline` snippets** on page, issue and comment search — `MaxFragments=2, MaxWords=20, MinWords=8, ShortWord=3`, delimited with Private Use Area sentinels (U+E000/U+E001) from `packages/shared/src/search.ts` rather than `ts_headline`'s `<b>` default, because the DTO is rendered as text by the web and read raw by agents (no HTML in JSON, no collision with the markdown being excerpted); the headline is computed in an OUTER select over a subquery that already applied `LIMIT`/`OFFSET`, since `ts_headline` re-parses the whole document per row. (2) **Real server-side pagination** replacing the hard-coded `RESULT_CAP = 20` + client-side slice that made match #21 unreachable — `limit`/`offset` (default 20, max 50) per group with a true `total` from `COUNT(*) OVER()` in the same query, plus `hasMore`; deterministic `ORDER BY rank DESC, id` so paging can't duplicate or skip. A new `?groups=` param computes only the requested groups. (3) **Comments are indexed** — migration `20260730120000_add_comment_fts` adds `Comment.searchVector` (generated `tsvector` + GIN, mirroring `Page`/`Issue`), making the place decisions are actually written down findable for the first time; scoped Comment → Issue → Project → workspace, gated by the route's existing `issues:read` (a comment is issue data). **Result for the same question: 1,112 B in 1 call (−94.4%)**; "what did we decide about rate limiting?" = 1 call / 441 B, previously unanswerable. MCP updated in lockstep: `search_pages`/`search_issues` now page server-side and their descriptions say so, `search_pages` drops the per-row `workspaceId` (making its already-published compact-shape description true), and a new `search_comments` tool exposes the new group. Gates: API unit 2102/2102, shared 194, MCP 137, integration 448/448 including a new tenant-isolation block proving neither the snippet TEXT nor the comment index crosses a tenant boundary; `tsc --noEmit` clean on api/web/mcp.
...
- ✅ **Kustomize base + overlays** as a Helm alternative (`deploy/kustomize/base` + `overlays/dev`, `overlays/prod`): same workloads as plain manifests, with replica/host/TLS/HPA/PDB patches per overlay. (2026-06-27 — authored; **actually rendered for the first time 2026-07-30**, which immediately exposed two bugs that "all manifests parse" could never have caught — see below.)
- ✅ **Kustomize deploy path repaired + gated in CI (2026-07-30).** Rendering the overlays for the first time proved the Kustomize path had never worked. **(1) Wrong registry namespace:** all six image refs pointed at `ghcr.io/next-lane/…`, which does not exist (we publish to `ghcr.io/overcastly-ai/…`) — verified against the live GHCR API (`403` vs. a real tag list). **(2) Tag pinned to a version never released:** base + prod pinned `1.0.0` while the project shipped `0.2.0`, so `kubectl apply -k` gave `ImagePullBackOff` on a perfectly green build. **(3) Inert dev overlay:** the dev overlay's `newTag: latest` silently no-op'd because a Kustomize image transformer matches the manifest's *current* image and the base had already rewritten the short name — dev was inheriting the base's pinned tag. Fixes: namespace corrected everywhere; the pinned Kustomize tags are now **owned by `scripts/sync-versions.mjs`** (11 version records, up from 7) so a release can't leave them behind; the dev overlay keys on the post-base image name. New **`Kustomize overlays render`** CI job renders both overlays and asserts the exact image set (owner derived at runtime, so forks pass) — the gate was verified to fail on a reintroduced regression, not just to pass. **(4) The release would have un-synced them again:** `release.config.mjs` runs the syncer in `prepareCmd` but its `@semantic-release/git.assets` list didn't include the new files, so every release would have rewritten them without committing them; both are now listed, and `sync-versions.mjs --files` + a new CI assertion (`assets ⊇ --files`) make that class of omission impossible to reintroduce silently. Proven live: v0.3.0 was auto-cut when #57 merged and the guard immediately caught the lagging tags. `docs/DEPLOY-KUBERNETES.md` corrected alongside: it still documented `--build-arg VITE_API_URL=…` baking the API origin into the bundle, which stopped being true when the web image moved to runtime `config.js`, and still called multi-arch GHCR publishing an unshipped "Phase 4 deliverable".
