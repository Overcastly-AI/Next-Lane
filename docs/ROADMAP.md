...
- ✅ Web nginx security headers (engineering-auditor Pass 5): `Content-Security-Policy` (+ `X-Content-Type-Options`/`X-Frame-Options`/`Referrer-Policy`) added to all three nginx configs (standalone `apps/web/nginx.conf`, Helm same-origin ConfigMap, Kustomize web-nginx ConfigMap) — `script-src 'self'`, same-origin `connect-src`, documented widening for external-API mode. Closes the SPA-has-no-CSP gap (the API already had it via `helmet()`).
- ✅ Helm bundled-Postgres default-password fail-fast (engineering-auditor Pass 5): chart now refuses to render when the bundled PG is enabled and `postgresql.auth.password` is empty/`nextlane`, mirroring the `secrets.jwtSecret` guard (`existingSecret` / `allowInsecurePassword` escape hatches; documented in values.yaml + DEPLOY-KUBERNETES.md).
