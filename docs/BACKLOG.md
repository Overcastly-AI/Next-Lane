...
- [~] (P1, M) Cross-project global search — search endpoint (title + description) across all projects the user is a member of; surfaced via header search bar or command palette; current search is single-project and only exposed in the parent-picker popover [product-auditor] — API done (`GET /search`, `GET /projects/:id/search`, membership-scoped); palette UI pending
