...
- [x] (P1, S) Emit `WATCHED_UPDATED` notification on issue field changes — `IssuesService.update` now fans out `WATCHED_UPDATED` to all watchers (minus actor) via `NotificationsService.notifyWatchersUpdated`; batched `createMany` insert (one DB round-trip regardless of watcher count) + per-user realtime push via `user:<id>` room; fires on status/assignee/priority/title/dueDate changes; no-op patches (same value) and non-meaningful fields (storyPoints etc.) are skipped; actor never notified; 11 new unit tests (6 in notifications.service.spec + 5 in issues.service.spec); 281 total unit tests green; shipped 2026-06-27 [product-auditor Pass 5]
...
- [ ] (P2, M) Batch notifyComment inserts (createMany) + rebalanceAndPlace (executeRaw batch UPDATE) — serial N inserts per watcher in notifyComment; serial N updates in rebalance tx; WATCHED_UPDATED fan-out already uses createMany (shipped 2026-06-27) [engineering-auditor Pass 4, Pass 5]
...
## Changelog
- 2026-06-27 — WATCHED_UPDATED notification emission (P1, S): `NotificationsService.notifyWatchersUpdated` added (batched `createMany` + per-user realtime push); `IssuesService.update` fans out on status/assignee/priority/title/dueDate changes; actor excluded; no-op skipped; 11 new unit tests; 281 total green.
