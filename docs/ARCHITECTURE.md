...
- **NestJS** with the standard module/controller/service/dto pattern. Modules by domain: `auth`, `users`, `workspaces`, `projects`, `boards`, `sprints`, `issues`, `custom-fields`, `components`, `versions`, `labels`, `comments`, `issue-links`, `issue-templates`, `checklist`, `work-logs`, `workflows`, `statuses`, `personal-boards`, `saved-filters`, `sprints`, `standups`, `poker`, `automations`, `dashboards`, `notifications`, `webhooks`, `share-tokens`, `api-tokens`, `analytics`, `reports`, `roadmap`, `attachments`, `audit`, `search`, `realtime`, `mail`, `redis`, `github`, `gitlab`, `admin-settings`, `project-memberships`, `agent-context`, and `prisma`. Shared utilities in `common` include SSRF-safe fetch with DNS-pinning (`ssrf-safe-fetch.ts`), idempotency utilities, and secret encryption. Auth includes an optional `oidc` sub-module (SSO/OIDC with generic provider discovery).
...
- **Model Context Protocol** server (stdio transport) with **104 tools** (46 read, 58 write).
