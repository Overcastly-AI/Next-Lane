/**
 * GiteaLinksSection ("Development")
 *
 * Rendered in the IssueDetailDrawer main column, directly beneath
 * `GitlabLinksSection` (GitHub → GitLab → Gitea — the three-provider
 * Development layout). Lists Gitea pull requests, commits, and branches that
 * reference this issue's key (e.g. "NL-123"), populated by the inbound Gitea
 * webhook. Entirely hidden when the project has no Gitea integration
 * configured OR the issue has no links yet — the
 * `GET /issues/:issueId/gitea-links` endpoint returns `[]` in both cases, so
 * a single "any links?" check covers it.
 *
 * Unlike `GithubLinksSection`/`GitlabLinksSection`, there is NO live-status
 * poll here — Gitea v1 is deliberately links-only (no outbound API client,
 * no auto-transition-on-merge; see `GiteaIntegration`'s schema comment). Rows
 * show the last webhook-delivered state only.
 */
import type { IssueGiteaLinkDto, GiteaLinkKind } from '@next-lane/shared';
import { useIssueGiteaLinks } from '@/api/gitea';
import { cn } from '@/lib/cn';

const KIND_LABEL: Record<GiteaLinkKind, string> = {
  PR: 'Pull request',
  COMMIT: 'Commit',
  BRANCH: 'Branch',
};

/** Same Octicon-derived glyphs as `GithubLinksSection` — Gitea calls it "pull request" too, so the iconography carries over unchanged. */
function KindIcon({ kind }: { kind: GiteaLinkKind }) {
  if (kind === 'PR') {
    return (
...
function LinkRow({ link }: { link: IssueGiteaLinkDto }) {
  const label =
    link.kind === 'PR'
      ? `#${link.externalId}`
      : link.kind === 'COMMIT'
  ...
  return (
    <li data-testid="gitea-link-row" className="flex items-center gap-2 py-1.5">
...
export function GiteaLinksSection({ issueId }: { issueId: string }) {
  const linksQuery = useIssueGiteaLinks(issueId);
  const links = linksQuery.data ?? [];

  // Hidden entirely while loading (avoids a flash for the common case where
  // Gitea isn't configured / there are no links) and once loaded when empty
  // — see file header comment.
  if (linksQuery.isLoading || links.length === 0) return null;

  return (
    <section data-testid="gitea-links-section" aria-label="Gitea Development">
