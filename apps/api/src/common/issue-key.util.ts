 ...
 * (e.g. "NL-123", "NL-45") out of free text (a commit message, PR/MR title,
 * description, or branch name).
 *
 * Shared by every SCM integration (GitHub, GitLab, and any future forge) so
 * they all use exactly the same scoping semantics — extracted here from its
 * original home in `github/github-issue-key.util.ts` (which now re-exports
 * this function, so its own behavior/import path is unchanged for existing
 * callers/tests).
 *
